...
<style>
    ...
    .main .block-container { background-color: #0a0a0a; padding: 1.5rem; }
    ...
    .percent-box-jogador { border-color: #4a9eff; }
    .percent-box-banca { border-color: #ff4d4d; }
    .percent-box-empate { border-color: #32cd32; }
    ...
    .percent-number-jogador { color: #4a9eff; text-shadow: 0 0 10px #4a9eff; }
    .percent-number-banca { color: #ff4d4d; text-shadow: 0 0 10px #ff4d4d; }
    .percent-number-empate { color: #32cd32; text-shadow: 0 0 10px #32cd32; }
    ...
    .historico-container {
        background-color: #0f0f0f; padding: 15px; border-radius: 12px;
...
# SEÇÃO 1: Configuração da Banca (Compacta)
col1, col2, col3, col4 = st.columns([2, 1, 1, 1])

with col1:
    banca_input = st.number_input("💰 Banca (R$)", value=500.0, min_value=0.0, step=50.0, format="%.2f")
...
if not st.session_state.sessao_ativa:
    st.warning("⚠️ Configure a banca e clique em 'Iniciar' para começar.")
    st.stop()
...
if st.session_state.aguardando_resultado:
    ...
    <div class="alerta-aguardando">
        ⚠️ AGUARDANDO RESULTADO | Direção: <strong>{st.session_state.ultima_direcao_sugerida}</strong>
    </div>
    """, unsafe_allow_html=True)

# SEÇÃO 2: Botões de Registro (Lado a Lado)
...
# SEÇÃO 3: Tabela de Porcentagens (Logo abaixo dos botões)
if st.session_state.historico:
    ...
    pct_j = (qtd_j / total) * 100
    pct_b = (qtd_b / total) * 100
    pct_e = (qtd_e / total) * 100
    
    st.markdown(f"""
    <div class="tabela-percentual">
        <div class="tabela-grid">
            <div class="percent-box percent-box-jogador">
                <div class="percent-label">🔵 JOGADOR</div>
                <div class="percent-number percent-number-jogador">{pct_j:.1f}%</div>
                <div class="percent-label">{qtd_j} de {total}</div>
            </div>
            <div class="percent-box percent-box-banca">
                <div class="percent-label">🔴 BANCA</div>
                <div class="percent-number percent-number-banca">{pct_b:.1f}%</div>
                <div class="percent-label">{qtd_b} de {total}</div>
            </div>
            <div class="percent-box percent-box-empate">
                <div class="percent-label">🟢 EMPATE</div>
                <div class="percent-number percent-number-empate">{pct_e:.1f}%</div>
                <div class="percent-label">{qtd_e} de {total}</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)

# SEÇÃO 4: Botão de Análise (Abaixo da tabela)
st.markdown('<div class="botao-analise-container">', unsafe_allow_html=True)
...
if pode_analisar:
    if st.button("🧠 GERAR ANÁLISE PRO MAX", key="btn_analise", use_container_width=True, type="primary"):
...
# SEÇÃO 5: Histórico Visual (Compacto)
st.markdown("### 📊 Histórico")

if not st.session_state.historico:
    st.info("Aguardando resultados...")
else:
    linhas_por_coluna = 8
    colunas = []
    coluna_atual = []
...
if st.session_state.ultima_analise_completa:
    ...
    <div class="score-card {classe_score}">
        <div style="color: #aaaaaa; font-size: 12px; letter-spacing: 3px;">SCORE DE CONFIANÇA</div>
        <div class="score-number">{score}</div>
        <div style="color: #888; font-size: 14px;">de 100 pontos</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="direcao-visual {classe_direcao}">
        <div class="direcao-titulo">🎯 DIREÇÃO SUGERIDA</div>
        <div class="direcao-icone">{icone}</div>
        <div class="direcao-texto">{texto_direcao}</div>
    </div>
    """, unsafe_allow_html=True)
    
    with st.expander("🔍 Ver Fatores Analisados"):
    ...
    with col2:
        st.metric("💵 Entrada (2%)", f"R$ {entrada_sugerida:.2f}")

elif st.session_state.analise_gerada:
    st.info("Registre pelo menos 10 resultados para gerar uma análise.")
else:
    st.info("🧠 Clique em 'GERAR ANÁLISE PRO MAX' para ativar a máquina.")

st.markdown("---")

# SEÇÃO 7: Estatísticas de Acertos
...
if stats['historico_resultados']:
    with st.expander("📝 Histórico Detalhado"):
        ...
        for i, (resultado, tipo) in enumerate(reversed(stats['historico_resultados'][-20:]), 1):
            ...
            historico_html += f'<div style="color: {cor}; padding: 8px; border-bottom: 1px solid #1a1a1a;">{icone} #{len(stats["historico_resultados"]) - i + 1}: {resultado} ({tipo})</div>'
        historico_html += '</div>'
